...
const upsert = useUpsert<StepModel>({
  ...
  items: [
    { field: "__step", label: "步骤", hidden: true },
    { field: "avatar", label: "头像", component: { is: "el-input", props: { placeholder: "请输入头像地址" } }, hidden: (model: StepModel) => model.__step !== "base" },
    { field: "name", label: "姓名", component: { is: "el-input", props: { placeholder: "请输入姓名" } }, rules: [{ required: true, message: "必填" }], hidden: (model: StepModel) => model.__step !== "base" },
    { field: "account", label: "账号", component: { is: "el-input", props: { placeholder: "请输入账号" } }, hidden: (model: StepModel) => model.__step !== "base" },
    {
      field: "priority",
      label: "优先级",
      component: {
    ...
    { field: "status", label: "状态", component: { is: "el-switch", props: { activeValue: 1, inactiveValue: 0 } }, hidden: (model: StepModel) => model.__step !== "base" },
    { field: "progress", label: "进度", component: { is: "el-slider", props: { min: 0, max: 100, showStops: true } }, hidden: (model: StepModel) => model.__step !== "extra" },
    { field: "score", label: "评分", component: { is: "el-rate", props: { allowHalf: true } }, hidden: (model: StepModel) => model.__step !== "extra" },
    { field: "website", label: "个人主页", component: { is: "el-input", props: { placeholder: "可选" } }, hidden: (model: StepModel) => model.__step !== "extra" },
    { field: "remark", label: "备注", span: 2, component: { is: "el-input", props: { type: "textarea", rows: 3 } }, hidden: (model: StepModel) => model.__step !== "extra" },
  ],
  onOpen: (_model, ctx) => {
    setStep(0)
