 ...
 * @param slots 组件插槽
 */
export function renderAction(engine: SearchCore, action: SearchAction, index: number, slots: Slots): VNodeChild {
    ...
    <FdGridItem key={index} class="fd-search__action-item" {...engine.getActionItemProps(action)}>
      {action.type === "search" && (
        <ElButton type="primary" disabled={loading.value} onClick={() => engine.search()}>
          <ElIcon class={["fd-search__icon", { "is-loading": loading.value }]}>{loading.value ? <IconLoading /> : <IconSearch />}</ElIcon>
      ...
      {action.type === "collapse" && (
        <ElButton onClick={() => engine.collapse()}>
          <ElIcon class="fd-search__icon">{!collapsed.value ? <IconArrowUp /> : <IconArrowDown />}</ElIcon>
...
export function renderActions(engine: SearchCore, slots: Slots): VNodeChild[] {
  return engine.resolvedActions.value.map((action, index) => renderAction(engine, action, index, slots))
}
