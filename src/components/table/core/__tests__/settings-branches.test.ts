...
describe("table/core/settings 分支补测", () => {
  describe("onDragMove", () => {
    ...
    it("固定列或 fixed 不一致时返回 false，其余情况返回 true", () => {
      const base = { sort: true, pinned: true, fixed: "left" as const }
      expect(onDragMove({ draggedContext: { element: base } } as any)).toBe(false)

      const dragged = { sort: true, pinned: false, fixed: "left" as const }
      const related = { sort: true, pinned: false, fixed: "right" as const }
      expect(onDragMove({ draggedContext: { element: dragged }, relatedContext: { element: related } })).toBe(false)

      const okDragged = { sort: true, pinned: false, fixed: "left" as const }
      const okRelated = { sort: true, pinned: false, fixed: "left" as const }
  ...
  describe("toggleFixed 与排序", () => {
    it("切换固定状态并保持排序结果稳定", () => {
      const state = createState([
        { id: "a", label: "A", show: true, order: 0, sort: true },
        { id: "b", label: "B", show: true, order: 1, sort: true },
      ])
      ...
      state.columnSettings.value = [
        { id: "b", label: "B", show: true, order: 0, sort: true },
        { id: "a", label: "A", show: true, order: 1, sort: true },
      ]
      sortColumnSettings(state)
      expect(state.columnSettings.value.map(item => item.id)).toEqual(["b", "a"])
