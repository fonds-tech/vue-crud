...
**Tests**: 所有新增/变更行为默认需要配套测试（Vitest 单测或组件测试，含成功与失败/边界场景）；仅当 spec 明确豁免时可省略，并须记录理由。
...
- [ ] T004 [P] Ensure test harness (Vitest + happy-dom) and typecheck scripts are wired into CI/pnpm scripts
...
- [ ] T010 Define UX state baselines (loading/empty/error/disabled) and shared tokens
- [ ] T011 Establish performance guardrails (e.g., dataset size targets, virtualization toggles, dependency audit)
